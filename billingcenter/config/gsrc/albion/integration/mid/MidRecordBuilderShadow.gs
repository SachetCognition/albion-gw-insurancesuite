...
/*
 * MidRecordBuilderShadow - dormant shadow / cut-over seam for the MID feed.
 *
 ...
 *     pass-through to MidRecordBuilder.buildRecord, byte-for-byte, no comparison work.
 *
 * Flags: bi.feature.shadow.midrecordbuilder.enabled and bi.feature.cutover.midrecordbuilder.enabled
 * (+ optional .brands / .brand.<CODE>.enabled overrides).
 */
class MidRecordBuilderShadow {

  public static final var CENTRE : String = AlbionFeatureFlags.CENTRE_BILLINGCENTER
  public static final var FEATURE : String = "shadow.midrecordbuilder"
  public static final var CUTOVER_FEATURE : String = "cutover.midrecordbuilder"
  public static final var FEED : String = "MID"
  ...
  public static final var BRAND_DRIFT_NOTE : String =
      "brand codes are maintained in MidRecordBuilder.brandMap() AND integration/polaris/mappings/brand_xref.csv; they have drifted before (AGI-5452 / AGI-30921); convergence is the separate brand stream"
  ...
  public static function buildRecord(src : KeyableBean, recorder : ShadowDiffRecorder) : String {
    ...
    if (AlbionFeatureFlags.isEnabledForBrand(CENTRE, CUTOVER_FEATURE, brandCode)) {
      return CutoverRouter.route(FEED, brandCode, recorder, BRAND_DRIFT_NOTE,
          \ -> MidRecordBuilder.buildRecord(src),
          \ -> MidRecordBuilderCandidate.buildRecord(src))
    }
    if (not AlbionFeatureFlags.isEnabledForBrand(CENTRE, FEATURE, brandCode)) {
      return MidRecordBuilder.buildRecord(src)   // dormant default: legacy only
    }
