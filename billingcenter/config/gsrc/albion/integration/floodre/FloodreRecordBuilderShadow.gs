...
/*
 * FloodreRecordBuilderShadow - dormant shadow / cut-over seam for the FLOODRE feed.
 *
 ...
 *     pass-through to FloodreRecordBuilder.buildRecord, byte-for-byte, no comparison work.
 *
 * Flags: bi.feature.shadow.floodrerecordbuilder.enabled and bi.feature.cutover.floodrerecordbuilder.enabled
 * (+ optional .brands / .brand.<CODE>.enabled overrides).
 */
class FloodreRecordBuilderShadow {

  public static final var CENTRE : String = AlbionFeatureFlags.CENTRE_BILLINGCENTER
  public static final var FEATURE : String = "shadow.floodrerecordbuilder"
  public static final var CUTOVER_FEATURE : String = "cutover.floodrerecordbuilder"
  public static final var FEED : String = "FLOODRE"
  ...
  public static final var BRAND_DRIFT_NOTE : String =
      "brand codes are maintained in FloodreRecordBuilder.brandMap() AND integration/polaris/mappings/brand_xref.csv; they have drifted before (AGI-5452 / AGI-30921); convergence is the separate brand stream"
  ...
  public static function buildRecord(src : KeyableBean, recorder : ShadowDiffRecorder) : String {
    ...
    if (AlbionFeatureFlags.isEnabledForBrand(CENTRE, CUTOVER_FEATURE, brandCode)) {
      return CutoverRouter.route(FEED, brandCode, recorder, BRAND_DRIFT_NOTE,
          \ -> FloodreRecordBuilder.buildRecord(src),
          \ -> FloodreRecordBuilderCandidate.buildRecord(src))
    }
    if (not AlbionFeatureFlags.isEnabledForBrand(CENTRE, FEATURE, brandCode)) {
      return FloodreRecordBuilder.buildRecord(src)   // dormant default: legacy only
    }
