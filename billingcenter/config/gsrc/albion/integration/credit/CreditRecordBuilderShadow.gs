...
/*
 * CreditRecordBuilderShadow - dormant shadow / cut-over seam for the CREDIT feed.
 *
 ...
 *     pass-through to CreditRecordBuilder.buildRecord, byte-for-byte, no comparison work.
 *
 * Flags: bi.feature.shadow.creditrecordbuilder.enabled and bi.feature.cutover.creditrecordbuilder.enabled
 * (+ optional .brands / .brand.<CODE>.enabled overrides).
 */
class CreditRecordBuilderShadow {

  public static final var CENTRE : String = AlbionFeatureFlags.CENTRE_BILLINGCENTER
  public static final var FEATURE : String = "shadow.creditrecordbuilder"
  public static final var CUTOVER_FEATURE : String = "cutover.creditrecordbuilder"
  public static final var FEED : String = "CREDIT"
  ...
  public static final var BRAND_DRIFT_NOTE : String =
      "brand codes are maintained in CreditRecordBuilder.brandMap() AND integration/polaris/mappings/brand_xref.csv; they have drifted before (AGI-5452 / AGI-30921); convergence is the separate brand stream"
  ...
  public static function buildRecord(src : KeyableBean, recorder : ShadowDiffRecorder) : String {
    ...
    if (AlbionFeatureFlags.isEnabledForBrand(CENTRE, CUTOVER_FEATURE, brandCode)) {
      return CutoverRouter.route(FEED, brandCode, recorder, BRAND_DRIFT_NOTE,
          \ -> CreditRecordBuilder.buildRecord(src),
          \ -> CreditRecordBuilderCandidate.buildRecord(src))
    }
    if (not AlbionFeatureFlags.isEnabledForBrand(CENTRE, FEATURE, brandCode)) {
      return CreditRecordBuilder.buildRecord(src)   // dormant default: legacy only
    }
