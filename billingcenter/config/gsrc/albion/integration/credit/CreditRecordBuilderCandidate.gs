...
/*
 * CreditRecordBuilderCandidate - Phase 2 CANDIDATE record builder for the CREDIT feed. NOT authoritative anywhere.
 *
 * Same bytes as CreditRecordBuilder, expressed as a field layout instead of a hand-transcribed
 * StringBuilder sequence. It is reached only through CreditRecordBuilderShadow, i.e. only when the
 * dormant "shadow.creditrecordbuilder" flag is turned on in a non-production environment; the
 * legacy CreditRecordBuilder output is what is returned and what production uses.
 *
 ...
 */
class CreditRecordBuilderCandidate {
  ...
  public static function buildRecord(src : KeyableBean) : String {
    return new FixedWidthRecordWriter(RECORD_LENGTH, FILLER_CHAR)
        .literal("CR61", 4)
        .text(src.getFieldValue("RiskPostcode_Ext") as String, 15)
        .text(src.getFieldValue("InceptionDate_Ext") as String, 30)
        .number(src.getFieldValue("PolicyNumber_Ext"), 15)
