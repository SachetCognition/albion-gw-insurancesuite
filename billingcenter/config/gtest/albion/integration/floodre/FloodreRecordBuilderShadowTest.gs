...
/*
 * FloodreRecordBuilderShadowTest - proves the Phase 2 scaffold for the FLOODRE feed is DORMANT, that the
 ...
 */
class FloodreRecordBuilderShadowTest extends TestBase {

  static final var CENTRE : String = FloodreRecordBuilderShadow.CENTRE
  static final var FEATURE : String = FloodreRecordBuilderShadow.FEATURE
  static final var CUTOVER : String = FloodreRecordBuilderShadow.CUTOVER_FEATURE

  function testFlagKeysFollowTheDocumentedConvention() {
    assertEquals("bi.feature.shadow.floodrerecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, FEATURE))
    assertEquals("bi.feature.shadow.floodrerecordbuilder.brands", AlbionFeatureFlags.brandsKey(CENTRE, FEATURE))
    assertEquals("bi.feature.shadow.floodrerecordbuilder.brand.ALBDIR.enabled",
        AlbionFeatureFlags.brandOverrideKey(CENTRE, FEATURE, "ALBDIR"))
    assertEquals("bi.feature.cutover.floodrerecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, CUTOVER))
  }
  ...
  function testDormantWithNoConfigurationForEveryBrand() {
    ...
    for (brand in brands) {
      ...
      assertEquals(legacyRecord(brand), FloodreRecordBuilderShadow.buildRecord(source(brand), recorder))
  ...
  function testExplicitlyDisabledFlagIsLegacyOnly() {
    ...
    var recorder = new InMemoryShadowDiffRecorder()
    assertEquals(legacyRecord("ALBDIR"), FloodreRecordBuilderShadow.buildRecord(source("ALBDIR"), recorder))
  ...
  function testEnabledCentreWideStillReturnsTheLegacyRecord() {
    ...
    AlbionFeatureFlags.useProperties(properties)
    var recorder = new InMemoryShadowDiffRecorder()
    var record = FloodreRecordBuilderShadow.buildRecord(source("ALBDIR"), recorder)
  ...
  function testBrandAllowListGatesTheShadowRun() {
    ...
    assertEquals(legacyRecord("HERIT"), FloodreRecordBuilderShadow.buildRecord(source("HERIT"), new InMemoryShadowDiffRecorder()))
  }
  ...
  function testPerBrandOverridePrecedence() {
    ...
    assertEquals(legacyRecord("RETPLS"), FloodreRecordBuilderShadow.buildRecord(source("RETPLS"), new InMemoryShadowDiffRecorder()))
  }
  ...
  function testCandidateIsByteIdenticalAcrossTheMatrix() {
    ...
    for (brand in brands) {
      ...
      reconcile(recorder, "AAAAAAAAAAAAAAAAA", "BBBBBBBBBBBBBBB", null, null, brand)
  ...
  function testLegacyFailurePropagatesUnchanged() {
    ...
    try {
      FloodreRecordBuilderShadow.reconcile(src, "HERIT", recorder)
  ...
  function testReconciliationCarriesTheBrandDriftNote() {
    var recorder = new InMemoryShadowDiffRecorder()
    var result = FloodreRecordBuilderShadow.reconcile(source("ALBDIR"), "ALBDIR", recorder)
    assertEquals(ReconciliationResult.MATCH, result.Outcome)
    assertEquals(legacyRecord("ALBDIR"), result.AuthoritativeOutput)
    assertEquals("FLOODRE", result.FeedName)
  ...
  function testCutoverFlagRoutesCandidateWithReverseShadowParity() {
    ...
    assertEquals(legacyRecord("HERIT"), FloodreRecordBuilderShadow.buildRecord(source("HERIT"), recorder))
    assertEquals(0, recorder.RecordCount)
  }
  ...
  private function reconcile(recorder : InMemoryShadowDiffRecorder, first : String, second : String,
    ...
    var result = FloodreRecordBuilderShadow.reconcile(source(first, second, number, date, brand), brand, recorder)
    assertEquals(FloodreRecordBuilder.buildRecord(source(first, second, number, date, brand)), result.AuthoritativeOutput)
  }

  private function legacyRecord(brand : String) : String {
    return FloodreRecordBuilder.buildRecord(source(brand))
  }
  ...
  private function source(first : String, second : String, number : BigDecimal,
    ...
    EasyMock.expect(mock.getFieldValue("SumInsured_Ext")).andReturn(number).anyTimes()
    EasyMock.expect(mock.getFieldValue("NINumber_Ext")).andReturn(date).anyTimes()
