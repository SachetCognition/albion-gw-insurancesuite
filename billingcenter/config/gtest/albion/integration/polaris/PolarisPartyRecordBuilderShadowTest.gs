...
class PolarisPartyRecordBuilderShadowTest extends TestBase {
  ...
  function testFlagKeysFollowTheDocumentedConvention() {
    assertEquals("bi.feature.shadow.polarispartyrecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, FEATURE))
    assertEquals("bi.feature.shadow.polarispartyrecordbuilder.brands", AlbionFeatureFlags.brandsKey(CENTRE, FEATURE))
    assertEquals("bi.feature.shadow.polarispartyrecordbuilder.brand.ALBDIR.enabled",
        AlbionFeatureFlags.brandOverrideKey(CENTRE, FEATURE, "ALBDIR"))
    assertEquals("bi.feature.cutover.polarispartyrecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, CUTOVER))
  }
  ...
  private function source(first : String, second : String, number : BigDecimal,
    ...
    EasyMock.expect(mock.getFieldValue("ERNRef_Ext")).andReturn(second).anyTimes()
