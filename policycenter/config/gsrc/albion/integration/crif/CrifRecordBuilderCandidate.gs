...
/*
 * CrifRecordBuilderCandidate - Phase 2 CANDIDATE record builder for the CRIF feed. NOT authoritative anywhere.
 *
 * Same bytes as CrifRecordBuilder, expressed as a field layout instead of a hand-transcribed
 * StringBuilder sequence. It is reached only through CrifRecordBuilderShadow, i.e. only when the
 * dormant "shadow.crifrecordbuilder" flag is turned on in a non-production environment; the
 * legacy CrifRecordBuilder output is what is returned and what production uses.
 *
 ...
 */
class CrifRecordBuilderCandidate {
  ...
  public static function buildRecord(src : KeyableBean) : String {
    return new FixedWidthRecordWriter(RECORD_LENGTH, FILLER_CHAR)
        .literal("CR58", 4)
        .text(src.getFieldValue("NINumber_Ext") as String, 15)
        .text(src.getFieldValue("VehicleVRM_Ext") as String, 30)
        .number(src.getFieldValue("AnnualPremium_Ext"), 15)
