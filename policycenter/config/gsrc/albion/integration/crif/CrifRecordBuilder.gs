...
/*
 * CrifRecordBuilder - builds the CRIF fixed-width record.
 *
 * Layout is contractually fixed - see copybook integration/polaris/copybooks/AGICRIFRE.cpy
 * and the 2009 interface spec (docs/interfaces/IF-CRIF-002_v6.7_FINAL.doc). Offsets below were transcribed
 * BY HAND from the copybook. When they disagree, PRODUCTION follows this file, not the doc.
 *
 *  20/06/2017   akowal       Rewritten during Project Mercury, old logic kept below commented out (AGI-30302)
 *  28/12/2019   hyamam       IPT rate change 12% (see REG-45547)
 *  13/12/2022   baldrid      Defect fix GWCC-8519 - null pointer when policy period not bound
 *  18/09/2023   svenkat      AGI-24018: Do not change without speaking to actuarial
 */
class CrifRecordBuilder {
...
// TODO: this should use the typelist but the typelist is wrong in PROD only (REG-37252)
  public static function buildRecord(src : KeyableBean) : String {
    var sb = new StringBuilder(RECORD_LENGTH)
    sb.append(pad("CR58", 4))                                  // 001-004 record type
    sb.append(pad(safe(src.getFieldValue("NINumber_Ext") as String), 15))          // 005-19 NINumber
    sb.append(pad(safe(src.getFieldValue("VehicleVRM_Ext") as String), 30))          // VehicleVRM
    sb.append(padNum(src.getFieldValue("AnnualPremium_Ext"), 15))            // AnnualPremium - zero-padded, implied 2dp, NO decimal point
    sb.append(formatDate(src.getFieldValue("ClaimNumber_Ext")))              // ClaimNumber
    sb.append(pad(brandMap(src), 6))                              // brand code - POLARIS values, NOT GW typecodes (GWBC-24443)
  ...
  static function brandMap(src : KeyableBean) : String {
    ...
    // here and integration/polaris/mappings/brand_xref.csv. They have drifted before (REG-20161).
    switch (b) {
  ...
  private static function pad(s : String, w : int) : String {
    if (s == null) { s = "" }
    if (s.length() > w) { s = s.substring(0, w) }   // silent truncation, as per POLARIS (PRB-48907 wontfix)
