...
/*
 * SspRecordBuilderShadow - dormant shadow / cut-over seam for the SSP feed.
 *
 ...
 *     pass-through to SspRecordBuilder.buildRecord, byte-for-byte, no comparison work.
 *
 * Flags: po.feature.shadow.ssprecordbuilder.enabled and po.feature.cutover.ssprecordbuilder.enabled
 * (+ optional .brands / .brand.<CODE>.enabled overrides).
 */
class SspRecordBuilderShadow {

  public static final var CENTRE : String = AlbionFeatureFlags.CENTRE_POLICYCENTER
  public static final var FEATURE : String = "shadow.ssprecordbuilder"
  public static final var CUTOVER_FEATURE : String = "cutover.ssprecordbuilder"
  public static final var FEED : String = "SSP"
  ...
  public static final var BRAND_DRIFT_NOTE : String =
      "brand codes are maintained in SspRecordBuilder.brandMap() AND integration/polaris/mappings/brand_xref.csv; they have drifted before (AGI-5452 / AGI-30921); convergence is the separate brand stream"
  ...
  public static function buildRecord(src : KeyableBean, recorder : ShadowDiffRecorder) : String {
    ...
    if (AlbionFeatureFlags.isEnabledForBrand(CENTRE, CUTOVER_FEATURE, brandCode)) {
      return CutoverRouter.route(FEED, brandCode, recorder, BRAND_DRIFT_NOTE,
          \ -> SspRecordBuilder.buildRecord(src),
          \ -> SspRecordBuilderCandidate.buildRecord(src))
    }
    if (not AlbionFeatureFlags.isEnabledForBrand(CENTRE, FEATURE, brandCode)) {
      return SspRecordBuilder.buildRecord(src)   // dormant default: legacy only
    }
