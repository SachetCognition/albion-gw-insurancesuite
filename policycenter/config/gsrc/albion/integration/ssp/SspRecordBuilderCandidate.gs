...
/*
 * SspRecordBuilderCandidate - Phase 2 CANDIDATE record builder for the SSP feed. NOT authoritative anywhere.
 *
 * Same bytes as SspRecordBuilder, expressed as a field layout instead of a hand-transcribed
 * StringBuilder sequence. It is reached only through SspRecordBuilderShadow, i.e. only when the
 * dormant "shadow.ssprecordbuilder" flag is turned on in a non-production environment; the
 * legacy SspRecordBuilder output is what is returned and what production uses.
 *
 * Every documented quirk is preserved on purpose (fixed 750-byte record and its length
 * guard, silent truncation, COBOL trailing-overpunch negatives, AnnualPremium_Ext rendered
 ...
 */
class SspRecordBuilderCandidate {
  ...
  public static function buildRecord(src : KeyableBean) : String {
    return new FixedWidthRecordWriter(RECORD_LENGTH, FILLER_CHAR)
        .literal("SS84", 4)
        .text(src.getFieldValue("ClaimNumber_Ext") as String, 15)
        .text(src.getFieldValue("VehicleVRM_Ext") as String, 10)
        .number(src.getFieldValue("PolicyNumber_Ext"), 11)
        .date(src.getFieldValue("AnnualPremium_Ext"), DATE_FMT)
