...
/*
 * VeriskRecordBuilderShadow - dormant shadow / cut-over seam for the VERISK feed.
 *
 ...
 *     pass-through to VeriskRecordBuilder.buildRecord, byte-for-byte, no comparison work.
 *
 * Flags: po.feature.shadow.veriskrecordbuilder.enabled and po.feature.cutover.veriskrecordbuilder.enabled
 * (+ optional .brands / .brand.<CODE>.enabled overrides).
 */
class VeriskRecordBuilderShadow {

  public static final var CENTRE : String = AlbionFeatureFlags.CENTRE_POLICYCENTER
  public static final var FEATURE : String = "shadow.veriskrecordbuilder"
  public static final var CUTOVER_FEATURE : String = "cutover.veriskrecordbuilder"
  public static final var FEED : String = "VERISK"
  ...
  public static final var BRAND_DRIFT_NOTE : String =
      "brand codes are maintained in VeriskRecordBuilder.brandMap() AND integration/polaris/mappings/brand_xref.csv; they have drifted before (AGI-5452 / AGI-30921); convergence is the separate brand stream"
  ...
  public static function buildRecord(src : KeyableBean, recorder : ShadowDiffRecorder) : String {
    ...
    if (AlbionFeatureFlags.isEnabledForBrand(CENTRE, CUTOVER_FEATURE, brandCode)) {
      return CutoverRouter.route(FEED, brandCode, recorder, BRAND_DRIFT_NOTE,
          \ -> VeriskRecordBuilder.buildRecord(src),
          \ -> VeriskRecordBuilderCandidate.buildRecord(src))
    }
    if (not AlbionFeatureFlags.isEnabledForBrand(CENTRE, FEATURE, brandCode)) {
      return VeriskRecordBuilder.buildRecord(src)   // dormant default: legacy only
    }
