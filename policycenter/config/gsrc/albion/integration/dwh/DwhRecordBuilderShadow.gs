...
/*
 * DwhRecordBuilderShadow - dormant shadow / cut-over seam for the DWH feed.
 *
 ...
 *     pass-through to DwhRecordBuilder.buildRecord, byte-for-byte, no comparison work.
 *
 * Flags: po.feature.shadow.dwhrecordbuilder.enabled and po.feature.cutover.dwhrecordbuilder.enabled
 * (+ optional .brands / .brand.<CODE>.enabled overrides).
 */
class DwhRecordBuilderShadow {

  public static final var CENTRE : String = AlbionFeatureFlags.CENTRE_POLICYCENTER
  public static final var FEATURE : String = "shadow.dwhrecordbuilder"
  public static final var CUTOVER_FEATURE : String = "cutover.dwhrecordbuilder"
  public static final var FEED : String = "DWH"
  ...
  public static final var BRAND_DRIFT_NOTE : String =
      "brand codes are maintained in DwhRecordBuilder.brandMap() AND integration/polaris/mappings/brand_xref.csv; they have drifted before (AGI-5452 / AGI-30921); convergence is the separate brand stream"
  ...
  public static function buildRecord(src : KeyableBean, recorder : ShadowDiffRecorder) : String {
    ...
    if (AlbionFeatureFlags.isEnabledForBrand(CENTRE, CUTOVER_FEATURE, brandCode)) {
      return CutoverRouter.route(FEED, brandCode, recorder, BRAND_DRIFT_NOTE,
          \ -> DwhRecordBuilder.buildRecord(src),
          \ -> DwhRecordBuilderCandidate.buildRecord(src))
    }
    if (not AlbionFeatureFlags.isEnabledForBrand(CENTRE, FEATURE, brandCode)) {
      return DwhRecordBuilder.buildRecord(src)   // dormant default: legacy only
    }
