...
class PolarisPartyRecordBuilderShadowTest extends TestBase {
  ...
  function testFlagKeysFollowTheDocumentedConvention() {
    assertEquals("cl.feature.shadow.polarispartyrecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, FEATURE))
    assertEquals("cl.feature.shadow.polarispartyrecordbuilder.brands", AlbionFeatureFlags.brandsKey(CENTRE, FEATURE))
    assertEquals("cl.feature.shadow.polarispartyrecordbuilder.brand.ALBDIR.enabled",
        AlbionFeatureFlags.brandOverrideKey(CENTRE, FEATURE, "ALBDIR"))
    assertEquals("cl.feature.cutover.polarispartyrecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, CUTOVER))
  }
  ...
  private function source(first : String, second : String, number : BigDecimal,
    ...
    EasyMock.expect(mock.getFieldValue("InsuredSurname_Ext")).andReturn(second).anyTimes()
