...
/*
 * EltoRecordBuilderShadowTest - proves the Phase 2 scaffold for the ELTO feed is DORMANT, that the
 ...
 */
class EltoRecordBuilderShadowTest extends TestBase {

  static final var CENTRE : String = EltoRecordBuilderShadow.CENTRE
  static final var FEATURE : String = EltoRecordBuilderShadow.FEATURE
  static final var CUTOVER : String = EltoRecordBuilderShadow.CUTOVER_FEATURE

  function testFlagKeysFollowTheDocumentedConvention() {
    assertEquals("cl.feature.shadow.eltorecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, FEATURE))
    assertEquals("cl.feature.shadow.eltorecordbuilder.brands", AlbionFeatureFlags.brandsKey(CENTRE, FEATURE))
    assertEquals("cl.feature.shadow.eltorecordbuilder.brand.ALBDIR.enabled",
        AlbionFeatureFlags.brandOverrideKey(CENTRE, FEATURE, "ALBDIR"))
    assertEquals("cl.feature.cutover.eltorecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, CUTOVER))
  }
  ...
  function testDormantWithNoConfigurationForEveryBrand() {
    ...
    for (brand in brands) {
      ...
      assertEquals(legacyRecord(brand), EltoRecordBuilderShadow.buildRecord(source(brand), recorder))
  ...
  function testExplicitlyDisabledFlagIsLegacyOnly() {
    ...
    var recorder = new InMemoryShadowDiffRecorder()
    assertEquals(legacyRecord("ALBDIR"), EltoRecordBuilderShadow.buildRecord(source("ALBDIR"), recorder))
  ...
  function testEnabledCentreWideStillReturnsTheLegacyRecord() {
    ...
    AlbionFeatureFlags.useProperties(properties)
    var recorder = new InMemoryShadowDiffRecorder()
    var record = EltoRecordBuilderShadow.buildRecord(source("ALBDIR"), recorder)
  ...
  function testBrandAllowListGatesTheShadowRun() {
    ...
    assertEquals(legacyRecord("HERIT"), EltoRecordBuilderShadow.buildRecord(source("HERIT"), new InMemoryShadowDiffRecorder()))
  }
  ...
  function testPerBrandOverridePrecedence() {
    ...
    assertEquals(legacyRecord("RETPLS"), EltoRecordBuilderShadow.buildRecord(source("RETPLS"), new InMemoryShadowDiffRecorder()))
  }
  ...
  function testLegacyFailurePropagatesUnchanged() {
    ...
    try {
      EltoRecordBuilderShadow.reconcile(src, "HERIT", recorder)
  ...
  function testCutoverFlagRoutesCandidateWithReverseShadowParity() {
    ...
    assertEquals(legacyRecord("HERIT"), EltoRecordBuilderShadow.buildRecord(source("HERIT"), recorder))
    assertEquals(0, recorder.RecordCount)
  }
  ...
  private function reconcile(recorder : InMemoryShadowDiffRecorder, first : String, second : String,
    ...
    var result = EltoRecordBuilderShadow.reconcile(source(first, second, number, date, brand), brand, recorder)
    assertEquals(EltoRecordBuilder.buildRecord(source(first, second, number, date, brand)), result.AuthoritativeOutput)
  }

  private function legacyRecord(brand : String) : String {
    return EltoRecordBuilder.buildRecord(source(brand))
  }
  ...
  private function source(first : String, second : String, number : BigDecimal,
    ...
    EasyMock.expect(mock.getFieldValue("UPRN_Ext")).andReturn(second).anyTimes()
