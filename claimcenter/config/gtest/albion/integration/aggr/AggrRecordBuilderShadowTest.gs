...
/*
 * AggrRecordBuilderShadowTest - proves the Phase 2 scaffold for the AGGR feed is DORMANT, that the
 ...
 */
class AggrRecordBuilderShadowTest extends TestBase {

  static final var CENTRE : String = AggrRecordBuilderShadow.CENTRE
  static final var FEATURE : String = AggrRecordBuilderShadow.FEATURE
  static final var CUTOVER : String = AggrRecordBuilderShadow.CUTOVER_FEATURE

  function testFlagKeysFollowTheDocumentedConvention() {
    assertEquals("cl.feature.shadow.aggrrecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, FEATURE))
    assertEquals("cl.feature.shadow.aggrrecordbuilder.brands", AlbionFeatureFlags.brandsKey(CENTRE, FEATURE))
    assertEquals("cl.feature.shadow.aggrrecordbuilder.brand.ALBDIR.enabled",
        AlbionFeatureFlags.brandOverrideKey(CENTRE, FEATURE, "ALBDIR"))
    assertEquals("cl.feature.cutover.aggrrecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, CUTOVER))
  }
  ...
  function testDormantWithNoConfigurationForEveryBrand() {
    ...
    for (brand in brands) {
      ...
      assertEquals(legacyRecord(brand), AggrRecordBuilderShadow.buildRecord(source(brand), recorder))
  ...
  function testExplicitlyDisabledFlagIsLegacyOnly() {
    ...
    var recorder = new InMemoryShadowDiffRecorder()
    assertEquals(legacyRecord("ALBDIR"), AggrRecordBuilderShadow.buildRecord(source("ALBDIR"), recorder))
  ...
  function testEnabledCentreWideStillReturnsTheLegacyRecord() {
    ...
    AlbionFeatureFlags.useProperties(properties)
    var recorder = new InMemoryShadowDiffRecorder()
    var record = AggrRecordBuilderShadow.buildRecord(source("ALBDIR"), recorder)
  ...
  function testBrandAllowListGatesTheShadowRun() {
    ...
    assertEquals(legacyRecord("HERIT"), AggrRecordBuilderShadow.buildRecord(source("HERIT"), new InMemoryShadowDiffRecorder()))
  }
  ...
  function testPerBrandOverridePrecedence() {
    ...
    assertEquals(legacyRecord("RETPLS"), AggrRecordBuilderShadow.buildRecord(source("RETPLS"), new InMemoryShadowDiffRecorder()))
  }
  ...
  function testLegacyFailurePropagatesUnchanged() {
    ...
    try {
      AggrRecordBuilderShadow.reconcile(src, "HERIT", recorder)
  ...
  function testCutoverFlagRoutesCandidateWithReverseShadowParity() {
    ...
    assertEquals(legacyRecord("HERIT"), AggrRecordBuilderShadow.buildRecord(source("HERIT"), recorder))
    assertEquals(0, recorder.RecordCount)
  }
  ...
  private function reconcile(recorder : InMemoryShadowDiffRecorder, first : String, second : String,
    ...
    var result = AggrRecordBuilderShadow.reconcile(source(first, second, number, date, brand), brand, recorder)
    assertEquals(AggrRecordBuilder.buildRecord(source(first, second, number, date, brand)), result.AuthoritativeOutput)
  }

  private function legacyRecord(brand : String) : String {
    return AggrRecordBuilder.buildRecord(source(brand))
  }
  ...
  private function source(first : String, second : String, number : BigDecimal,
    ...
    EasyMock.expect(mock.getFieldValue("InsuredSurname_Ext")).andReturn(number).anyTimes()
