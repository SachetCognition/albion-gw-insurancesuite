...
/*
 * SanctionsRecordBuilderShadowTest - proves the Phase 2 scaffold for the SANCTIONS feed is DORMANT, that the
 ...
 */
class SanctionsRecordBuilderShadowTest extends TestBase {

  static final var CENTRE : String = SanctionsRecordBuilderShadow.CENTRE
  static final var FEATURE : String = SanctionsRecordBuilderShadow.FEATURE
  static final var CUTOVER : String = SanctionsRecordBuilderShadow.CUTOVER_FEATURE

  function testFlagKeysFollowTheDocumentedConvention() {
    assertEquals("cl.feature.shadow.sanctionsrecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, FEATURE))
    assertEquals("cl.feature.shadow.sanctionsrecordbuilder.brands", AlbionFeatureFlags.brandsKey(CENTRE, FEATURE))
    assertEquals("cl.feature.shadow.sanctionsrecordbuilder.brand.ALBDIR.enabled",
        AlbionFeatureFlags.brandOverrideKey(CENTRE, FEATURE, "ALBDIR"))
    assertEquals("cl.feature.cutover.sanctionsrecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, CUTOVER))
  }
  ...
  function testDormantWithNoConfigurationForEveryBrand() {
    ...
    for (brand in brands) {
      ...
      assertEquals(legacyRecord(brand), SanctionsRecordBuilderShadow.buildRecord(source(brand), recorder))
  ...
  function testExplicitlyDisabledFlagIsLegacyOnly() {
    ...
    var recorder = new InMemoryShadowDiffRecorder()
    assertEquals(legacyRecord("ALBDIR"), SanctionsRecordBuilderShadow.buildRecord(source("ALBDIR"), recorder))
  ...
  function testEnabledCentreWideStillReturnsTheLegacyRecord() {
    ...
    AlbionFeatureFlags.useProperties(properties)
    var recorder = new InMemoryShadowDiffRecorder()
    var record = SanctionsRecordBuilderShadow.buildRecord(source("ALBDIR"), recorder)
  ...
  function testBrandAllowListGatesTheShadowRun() {
    ...
    assertEquals(legacyRecord("HERIT"), SanctionsRecordBuilderShadow.buildRecord(source("HERIT"), new InMemoryShadowDiffRecorder()))
  }
  ...
  function testPerBrandOverridePrecedence() {
    ...
    assertEquals(legacyRecord("RETPLS"), SanctionsRecordBuilderShadow.buildRecord(source("RETPLS"), new InMemoryShadowDiffRecorder()))
  }
  ...
  function testCandidateIsByteIdenticalAcrossTheMatrix() {
    ...
    for (brand in brands) {
      ...
      reconcile(recorder, "AAAAAAAAAAAAAAAAAAAAAA", "BBBBBBBBBBBBBBB", null, null, brand)
  ...
  function testLegacyFailurePropagatesUnchanged() {
    ...
    try {
      SanctionsRecordBuilderShadow.reconcile(src, "HERIT", recorder)
  ...
  function testReconciliationCarriesTheBrandDriftNote() {
    var recorder = new InMemoryShadowDiffRecorder()
    var result = SanctionsRecordBuilderShadow.reconcile(source("ALBDIR"), "ALBDIR", recorder)
    assertEquals(ReconciliationResult.MATCH, result.Outcome)
    assertEquals(legacyRecord("ALBDIR"), result.AuthoritativeOutput)
    assertEquals("SANCTIONS", result.FeedName)
  ...
  function testCutoverFlagRoutesCandidateWithReverseShadowParity() {
    ...
    assertEquals(legacyRecord("HERIT"), SanctionsRecordBuilderShadow.buildRecord(source("HERIT"), recorder))
    assertEquals(0, recorder.RecordCount)
  }
  ...
  private function reconcile(recorder : InMemoryShadowDiffRecorder, first : String, second : String,
    ...
    var result = SanctionsRecordBuilderShadow.reconcile(source(first, second, number, date, brand), brand, recorder)
    assertEquals(SanctionsRecordBuilder.buildRecord(source(first, second, number, date, brand)), result.AuthoritativeOutput)
  }

  private function legacyRecord(brand : String) : String {
    return SanctionsRecordBuilder.buildRecord(source(brand))
  }
  ...
  private function source(first : String, second : String, number : BigDecimal,
    ...
    EasyMock.expect(mock.getFieldValue("PolicyNumber_Ext")).andReturn(first).anyTimes()
    EasyMock.expect(mock.getFieldValue("AnnualPremium_Ext")).andReturn(second).anyTimes()
    EasyMock.expect(mock.getFieldValue("UPRN_Ext")).andReturn(number).anyTimes()
    EasyMock.expect(mock.getFieldValue("RiskPostcode_Ext")).andReturn(date).anyTimes()
