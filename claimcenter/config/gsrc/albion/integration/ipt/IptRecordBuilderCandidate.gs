...
/*
 * IptRecordBuilderCandidate - Phase 1 CANDIDATE record builder. NOT authoritative anywhere.
 *
 * Same bytes, expressed as a field layout instead of a hand-transcribed StringBuilder
 * sequence. It is reached only through IptRecordBuilderShadow, i.e. only when the
 * dormant "shadow.iptrecordbuilder" flag is turned on in a non-production environment; the
 * legacy IptRecordBuilder output is what is returned and what production uses.
 *
 ...
 */
class IptRecordBuilderCandidate {
  ...
  public static function buildRecord(src : KeyableBean) : String {
    return new FixedWidthRecordWriter(RECORD_LENGTH, FILLER_CHAR)
        .literal("IP89", 4)
        .text(src.getFieldValue("ERNRef_Ext") as String, 20)
        .text(src.getFieldValue("PolicyNumber_Ext") as String, 30)
        .number(src.getFieldValue("NINumber_Ext"), 11)
