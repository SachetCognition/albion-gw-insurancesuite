...
/*
 * EltoRecordBuilderShadow - dormant shadow / cut-over seam for the ELTO feed.
 *
 ...
 *     pass-through to EltoRecordBuilder.buildRecord, byte-for-byte, no comparison work.
 *
 * Flags: cl.feature.shadow.eltorecordbuilder.enabled and cl.feature.cutover.eltorecordbuilder.enabled
 * (+ optional .brands / .brand.<CODE>.enabled overrides).
 */
class EltoRecordBuilderShadow {

  public static final var CENTRE : String = AlbionFeatureFlags.CENTRE_CLAIMCENTER
  public static final var FEATURE : String = "shadow.eltorecordbuilder"
  public static final var CUTOVER_FEATURE : String = "cutover.eltorecordbuilder"
  public static final var FEED : String = "ELTO"
  ...
  public static final var BRAND_DRIFT_NOTE : String =
      "brand codes are maintained in EltoRecordBuilder.brandMap() AND integration/polaris/mappings/brand_xref.csv; they have drifted before (AGI-5452 / AGI-30921); convergence is the separate brand stream"
  ...
  public static function buildRecord(src : KeyableBean, recorder : ShadowDiffRecorder) : String {
    ...
    if (AlbionFeatureFlags.isEnabledForBrand(CENTRE, CUTOVER_FEATURE, brandCode)) {
      return CutoverRouter.route(FEED, brandCode, recorder, BRAND_DRIFT_NOTE,
          \ -> EltoRecordBuilder.buildRecord(src),
          \ -> EltoRecordBuilderCandidate.buildRecord(src))
    }
    if (not AlbionFeatureFlags.isEnabledForBrand(CENTRE, FEATURE, brandCode)) {
      return EltoRecordBuilder.buildRecord(src)   // dormant default: legacy only
    }
