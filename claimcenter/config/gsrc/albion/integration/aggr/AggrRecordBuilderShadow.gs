...
/*
 * AggrRecordBuilderShadow - dormant shadow / cut-over seam for the AGGR feed.
 *
 ...
 *     pass-through to AggrRecordBuilder.buildRecord, byte-for-byte, no comparison work.
 *
 * Flags: cl.feature.shadow.aggrrecordbuilder.enabled and cl.feature.cutover.aggrrecordbuilder.enabled
 * (+ optional .brands / .brand.<CODE>.enabled overrides).
 */
class AggrRecordBuilderShadow {

  public static final var CENTRE : String = AlbionFeatureFlags.CENTRE_CLAIMCENTER
  public static final var FEATURE : String = "shadow.aggrrecordbuilder"
  public static final var CUTOVER_FEATURE : String = "cutover.aggrrecordbuilder"
  public static final var FEED : String = "AGGR"
  ...
  public static final var BRAND_DRIFT_NOTE : String =
      "brand codes are maintained in AggrRecordBuilder.brandMap() AND integration/polaris/mappings/brand_xref.csv; they have drifted before (AGI-5452 / AGI-30921); convergence is the separate brand stream"
  ...
  public static function buildRecord(src : KeyableBean, recorder : ShadowDiffRecorder) : String {
    ...
    if (AlbionFeatureFlags.isEnabledForBrand(CENTRE, CUTOVER_FEATURE, brandCode)) {
      return CutoverRouter.route(FEED, brandCode, recorder, BRAND_DRIFT_NOTE,
          \ -> AggrRecordBuilder.buildRecord(src),
          \ -> AggrRecordBuilderCandidate.buildRecord(src))
    }
    if (not AlbionFeatureFlags.isEnabledForBrand(CENTRE, FEATURE, brandCode)) {
      return AggrRecordBuilder.buildRecord(src)   // dormant default: legacy only
    }
