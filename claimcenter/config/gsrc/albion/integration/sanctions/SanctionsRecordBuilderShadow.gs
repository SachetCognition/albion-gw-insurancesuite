...
/*
 * SanctionsRecordBuilderShadow - dormant shadow / cut-over seam for the SANCTIONS feed.
 *
 ...
 *     pass-through to SanctionsRecordBuilder.buildRecord, byte-for-byte, no comparison work.
 *
 * Flags: cl.feature.shadow.sanctionsrecordbuilder.enabled and cl.feature.cutover.sanctionsrecordbuilder.enabled
 * (+ optional .brands / .brand.<CODE>.enabled overrides).
 */
class SanctionsRecordBuilderShadow {

  public static final var CENTRE : String = AlbionFeatureFlags.CENTRE_CLAIMCENTER
  public static final var FEATURE : String = "shadow.sanctionsrecordbuilder"
  public static final var CUTOVER_FEATURE : String = "cutover.sanctionsrecordbuilder"
  public static final var FEED : String = "SANCTIONS"
  ...
  public static final var BRAND_DRIFT_NOTE : String =
      "brand codes are maintained in SanctionsRecordBuilder.brandMap() AND integration/polaris/mappings/brand_xref.csv; they have drifted before (AGI-5452 / AGI-30921); convergence is the separate brand stream"
  ...
  public static function buildRecord(src : KeyableBean, recorder : ShadowDiffRecorder) : String {
    ...
    if (AlbionFeatureFlags.isEnabledForBrand(CENTRE, CUTOVER_FEATURE, brandCode)) {
      return CutoverRouter.route(FEED, brandCode, recorder, BRAND_DRIFT_NOTE,
          \ -> SanctionsRecordBuilder.buildRecord(src),
          \ -> SanctionsRecordBuilderCandidate.buildRecord(src))
    }
    if (not AlbionFeatureFlags.isEnabledForBrand(CENTRE, FEATURE, brandCode)) {
      return SanctionsRecordBuilder.buildRecord(src)   // dormant default: legacy only
    }
