...
/*
 * SanctionsRecordBuilderCandidate - Phase 2 CANDIDATE record builder for the SANCTIONS feed. NOT authoritative anywhere.
 *
 * Same bytes as SanctionsRecordBuilder, expressed as a field layout instead of a hand-transcribed
 * StringBuilder sequence. It is reached only through SanctionsRecordBuilderShadow, i.e. only when the
 * dormant "shadow.sanctionsrecordbuilder" flag is turned on in a non-production environment; the
 * legacy SanctionsRecordBuilder output is what is returned and what production uses.
 *
 ...
 */
class SanctionsRecordBuilderCandidate {
  ...
  public static function buildRecord(src : KeyableBean) : String {
    return new FixedWidthRecordWriter(RECORD_LENGTH, FILLER_CHAR)
        .literal("SA37", 4)
        .text(src.getFieldValue("PolicyNumber_Ext") as String, 15)
