...
/*
 * CifasRecordBuilderShadowTest - proves the Phase 2 scaffold for the CIFAS feed is DORMANT, that the
 ...
 */
class CifasRecordBuilderShadowTest extends TestBase {

  static final var CENTRE : String = CifasRecordBuilderShadow.CENTRE
  static final var FEATURE : String = CifasRecordBuilderShadow.FEATURE
  static final var CUTOVER : String = CifasRecordBuilderShadow.CUTOVER_FEATURE

  function testFlagKeysFollowTheDocumentedConvention() {
    assertEquals("co.feature.shadow.cifasrecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, FEATURE))
    assertEquals("co.feature.shadow.cifasrecordbuilder.brands", AlbionFeatureFlags.brandsKey(CENTRE, FEATURE))
    assertEquals("co.feature.shadow.cifasrecordbuilder.brand.ALBDIR.enabled",
        AlbionFeatureFlags.brandOverrideKey(CENTRE, FEATURE, "ALBDIR"))
    assertEquals("co.feature.cutover.cifasrecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, CUTOVER))
  }
  ...
  function testDormantWithNoConfigurationForEveryBrand() {
    ...
    for (brand in brands) {
      ...
      assertEquals(legacyRecord(brand), CifasRecordBuilderShadow.buildRecord(source(brand), recorder))
  ...
  function testExplicitlyDisabledFlagIsLegacyOnly() {
    ...
    var recorder = new InMemoryShadowDiffRecorder()
    assertEquals(legacyRecord("ALBDIR"), CifasRecordBuilderShadow.buildRecord(source("ALBDIR"), recorder))
  ...
  function testEnabledCentreWideStillReturnsTheLegacyRecord() {
    ...
    AlbionFeatureFlags.useProperties(properties)
    var recorder = new InMemoryShadowDiffRecorder()
    var record = CifasRecordBuilderShadow.buildRecord(source("ALBDIR"), recorder)
  ...
  function testBrandAllowListGatesTheShadowRun() {
    ...
    assertEquals(legacyRecord("HERIT"), CifasRecordBuilderShadow.buildRecord(source("HERIT"), new InMemoryShadowDiffRecorder()))
  }
  ...
  function testPerBrandOverridePrecedence() {
    ...
    assertEquals(legacyRecord("RETPLS"), CifasRecordBuilderShadow.buildRecord(source("RETPLS"), new InMemoryShadowDiffRecorder()))
  }
  ...
  function testCandidateIsByteIdenticalAcrossTheMatrix() {
    ...
    for (brand in brands) {
      ...
      reconcile(recorder, "AAAAAAAAAAAAAAAAAAA", "BBBBBBBBBBBBBBBBB", null, null, brand)
  ...
  function testLegacyFailurePropagatesUnchanged() {
    ...
    try {
      CifasRecordBuilderShadow.reconcile(src, "HERIT", recorder)
  ...
  function testCutoverFlagRoutesCandidateWithReverseShadowParity() {
    ...
    assertEquals(legacyRecord("HERIT"), CifasRecordBuilderShadow.buildRecord(source("HERIT"), recorder))
    assertEquals(0, recorder.RecordCount)
  }
  ...
  private function reconcile(recorder : InMemoryShadowDiffRecorder, first : String, second : String,
    ...
    var result = CifasRecordBuilderShadow.reconcile(source(first, second, number, date, brand), brand, recorder)
    assertEquals(CifasRecordBuilder.buildRecord(source(first, second, number, date, brand)), result.AuthoritativeOutput)
  }

  private function legacyRecord(brand : String) : String {
    return CifasRecordBuilder.buildRecord(source(brand))
  }
  ...
  private function source(first : String, second : String, number : BigDecimal,
    ...
    EasyMock.expect(mock.getFieldValue("NINumber_Ext")).andReturn(first).anyTimes()
    EasyMock.expect(mock.getFieldValue("SumInsured_Ext")).andReturn(second).anyTimes()
    EasyMock.expect(mock.getFieldValue("InceptionDate_Ext")).andReturn(number).anyTimes()
    EasyMock.expect(mock.getFieldValue("InsuredSurname_Ext")).andReturn(date).anyTimes()
