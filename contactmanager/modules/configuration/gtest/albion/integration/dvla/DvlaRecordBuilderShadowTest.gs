...
/*
 * DvlaRecordBuilderShadowTest - proves the Phase 2 scaffold for the DVLA feed is DORMANT, that the
 ...
 */
class DvlaRecordBuilderShadowTest extends TestBase {

  static final var CENTRE : String = DvlaRecordBuilderShadow.CENTRE
  static final var FEATURE : String = DvlaRecordBuilderShadow.FEATURE
  static final var CUTOVER : String = DvlaRecordBuilderShadow.CUTOVER_FEATURE

  function testFlagKeysFollowTheDocumentedConvention() {
    assertEquals("co.feature.shadow.dvlarecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, FEATURE))
    assertEquals("co.feature.shadow.dvlarecordbuilder.brands", AlbionFeatureFlags.brandsKey(CENTRE, FEATURE))
    assertEquals("co.feature.shadow.dvlarecordbuilder.brand.ALBDIR.enabled",
        AlbionFeatureFlags.brandOverrideKey(CENTRE, FEATURE, "ALBDIR"))
    assertEquals("co.feature.cutover.dvlarecordbuilder.enabled", AlbionFeatureFlags.enabledKey(CENTRE, CUTOVER))
  }
  ...
  function testDormantWithNoConfigurationForEveryBrand() {
    ...
    for (brand in brands) {
      ...
      assertEquals(legacyRecord(brand), DvlaRecordBuilderShadow.buildRecord(source(brand), recorder))
  ...
  function testExplicitlyDisabledFlagIsLegacyOnly() {
    ...
    var recorder = new InMemoryShadowDiffRecorder()
    assertEquals(legacyRecord("ALBDIR"), DvlaRecordBuilderShadow.buildRecord(source("ALBDIR"), recorder))
  ...
  function testEnabledCentreWideStillReturnsTheLegacyRecord() {
    ...
    AlbionFeatureFlags.useProperties(properties)
    var recorder = new InMemoryShadowDiffRecorder()
    var record = DvlaRecordBuilderShadow.buildRecord(source("ALBDIR"), recorder)
  ...
  function testBrandAllowListGatesTheShadowRun() {
    ...
    assertEquals(legacyRecord("HERIT"), DvlaRecordBuilderShadow.buildRecord(source("HERIT"), new InMemoryShadowDiffRecorder()))
  }
  ...
  function testPerBrandOverridePrecedence() {
    ...
    assertEquals(legacyRecord("RETPLS"), DvlaRecordBuilderShadow.buildRecord(source("RETPLS"), new InMemoryShadowDiffRecorder()))
  }
  ...
  function testCandidateIsByteIdenticalAcrossTheMatrix() {
    ...
    for (brand in brands) {
      ...
      reconcile(recorder, "AAAAAAAAAAAAAAAAAAA", "BBBBBBBBBBBBBBB", null, null, brand)
  ...
  function testLegacyFailurePropagatesUnchanged() {
    ...
    try {
      DvlaRecordBuilderShadow.reconcile(src, "HERIT", recorder)
  ...
  function testCutoverFlagRoutesCandidateWithReverseShadowParity() {
    ...
    assertEquals(legacyRecord("HERIT"), DvlaRecordBuilderShadow.buildRecord(source("HERIT"), recorder))
    assertEquals(0, recorder.RecordCount)
  }
  ...
  private function reconcile(recorder : InMemoryShadowDiffRecorder, first : String, second : String,
    ...
    var result = DvlaRecordBuilderShadow.reconcile(source(first, second, number, date, brand), brand, recorder)
    assertEquals(DvlaRecordBuilder.buildRecord(source(first, second, number, date, brand)), result.AuthoritativeOutput)
  }

  private function legacyRecord(brand : String) : String {
    return DvlaRecordBuilder.buildRecord(source(brand))
  }
  ...
  private function source(first : String, second : String, number : BigDecimal,
    ...
    EasyMock.expect(mock.getFieldValue("InceptionDate_Ext")).andReturn(first).anyTimes()
    EasyMock.expect(mock.getFieldValue("SumInsured_Ext")).andReturn(second).anyTimes()
    EasyMock.expect(mock.getFieldValue("RiskPostcode_Ext")).andReturn(number).anyTimes()
    EasyMock.expect(mock.getFieldValue("ERNRef_Ext")).andReturn(date).anyTimes()
