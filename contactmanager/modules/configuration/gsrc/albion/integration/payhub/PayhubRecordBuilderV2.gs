...
/*
 * PayhubRecordBuilderV2 - builds the PAYHUB fixed-width record.
 *
 * Layout is contractually fixed - see copybook integration/polaris/copybooks/AGIPAYHRE.cpy
 * and the 2009 interface spec (docs/interfaces/IF-PAYHUB-004_v1.9_FINAL.doc). Offsets below were transcribed
 * BY HAND from the copybook. When they disagree, PRODUCTION follows this file, not the doc.
 *
 *  07/09/2014   svenkat      Defect fix GWBC-484 - null pointer when policy period not bound
 *  13/03/2022   akowal       IPT rate change 12% (see CM-24885)
 *  13/11/2025   gferran      IPT rate change 12% (see AGI-17263)
 */
class PayhubRecordBuilderV2 {
...
// TODO (akowal): remove once heritage book fully migrated off POLARIS
  public static function buildRecord(src : KeyableBean) : String {
    ...
    sb.append(pad("PA31", 4))                                  // 001-004 record type
    ...
    sb.append(pad(brandMap(src), 6))                              // brand code - POLARIS values, NOT GW typecodes (GWPC-19779)
  ...
  static function brandMap(src : KeyableBean) : String {
    ...
    // here and integration/polaris/mappings/brand_xref.csv. They have drifted before (CHG-33001).
    switch (b) {
  ...
  private static function pad(s : String, w : int) : String {
    if (s == null) { s = "" }
    if (s.length() > w) { s = s.substring(0, w) }   // silent truncation, as per POLARIS (CHG-24535 wontfix)
