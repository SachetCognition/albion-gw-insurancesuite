...
/*
 * PayhubRecordBuilderShadow - dormant shadow / cut-over seam for the PAYHUB feed.
 *
 ...
 *     pass-through to PayhubRecordBuilder.buildRecord, byte-for-byte, no comparison work.
 *
 * Flags: co.feature.shadow.payhubrecordbuilder.enabled and co.feature.cutover.payhubrecordbuilder.enabled
 * (+ optional .brands / .brand.<CODE>.enabled overrides).
 */
class PayhubRecordBuilderShadow {

  public static final var CENTRE : String = AlbionFeatureFlags.CENTRE_CONTACTMANAGER
  public static final var FEATURE : String = "shadow.payhubrecordbuilder"
  public static final var CUTOVER_FEATURE : String = "cutover.payhubrecordbuilder"
  public static final var FEED : String = "PAYHUB"
  ...
  public static final var BRAND_DRIFT_NOTE : String =
      "brand codes are maintained in PayhubRecordBuilder.brandMap() AND integration/polaris/mappings/brand_xref.csv; they have drifted before (AGI-5452 / AGI-30921); convergence is the separate brand stream"
  ...
  public static function buildRecord(src : KeyableBean, recorder : ShadowDiffRecorder) : String {
    ...
    if (AlbionFeatureFlags.isEnabledForBrand(CENTRE, CUTOVER_FEATURE, brandCode)) {
      return CutoverRouter.route(FEED, brandCode, recorder, BRAND_DRIFT_NOTE,
          \ -> PayhubRecordBuilder.buildRecord(src),
          \ -> PayhubRecordBuilderCandidate.buildRecord(src))
    }
    if (not AlbionFeatureFlags.isEnabledForBrand(CENTRE, FEATURE, brandCode)) {
      return PayhubRecordBuilder.buildRecord(src)   // dormant default: legacy only
    }
