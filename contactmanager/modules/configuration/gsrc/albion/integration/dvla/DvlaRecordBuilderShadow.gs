...
/*
 * DvlaRecordBuilderShadow - dormant shadow / cut-over seam for the DVLA feed.
 *
 ...
 *     pass-through to DvlaRecordBuilder.buildRecord, byte-for-byte, no comparison work.
 *
 * Flags: co.feature.shadow.dvlarecordbuilder.enabled and co.feature.cutover.dvlarecordbuilder.enabled
 * (+ optional .brands / .brand.<CODE>.enabled overrides).
 */
class DvlaRecordBuilderShadow {

  public static final var CENTRE : String = AlbionFeatureFlags.CENTRE_CONTACTMANAGER
  public static final var FEATURE : String = "shadow.dvlarecordbuilder"
  public static final var CUTOVER_FEATURE : String = "cutover.dvlarecordbuilder"
  public static final var FEED : String = "DVLA"
  ...
  public static final var BRAND_DRIFT_NOTE : String =
      "brand codes are maintained in DvlaRecordBuilder.brandMap() AND integration/polaris/mappings/brand_xref.csv; they have drifted before (AGI-5452 / AGI-30921); convergence is the separate brand stream"
  ...
  public static function buildRecord(src : KeyableBean, recorder : ShadowDiffRecorder) : String {
    ...
    if (AlbionFeatureFlags.isEnabledForBrand(CENTRE, CUTOVER_FEATURE, brandCode)) {
      return CutoverRouter.route(FEED, brandCode, recorder, BRAND_DRIFT_NOTE,
          \ -> DvlaRecordBuilder.buildRecord(src),
          \ -> DvlaRecordBuilderCandidate.buildRecord(src))
    }
    if (not AlbionFeatureFlags.isEnabledForBrand(CENTRE, FEATURE, brandCode)) {
      return DvlaRecordBuilder.buildRecord(src)   // dormant default: legacy only
    }
