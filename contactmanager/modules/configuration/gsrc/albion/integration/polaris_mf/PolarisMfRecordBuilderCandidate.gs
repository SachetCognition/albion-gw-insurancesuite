...
/*
 * PolarisMfRecordBuilderCandidate - Phase 1 CANDIDATE record builder. NOT authoritative anywhere.
 *
 * Same bytes, expressed as a field layout instead of a hand-transcribed StringBuilder
 * sequence. It is reached only through PolarisMfRecordBuilderShadow, i.e. only when the
 * dormant "shadow.polarismfrecordbuilder" flag is turned on in a non-production environment; the
 * legacy PolarisMfRecordBuilder output is what is returned and what production uses.
 *
 * Every documented quirk is preserved on purpose (fixed 512-byte record and its length
 * guard, silent truncation, COBOL trailing-overpunch negatives, InceptionDate_Ext rendered
 ...
 * shadow difference, which is the opposite of the Phase 1 objective.
 *
 ...
 * brand-consolidation stream (AGI-30921 / AGI-5452).
 */
class PolarisMfRecordBuilderCandidate {
  ...
  public static function buildRecord(src : KeyableBean) : String {
    return new FixedWidthRecordWriter(RECORD_LENGTH, FILLER_CHAR)
        .literal("PO99", 4)
        .text(src.getFieldValue("InsuredSurname_Ext") as String, 12)
        .text(src.getFieldValue("VehicleVRM_Ext") as String, 30)
        .number(src.getFieldValue("ClaimNumber_Ext"), 15)
        .date(src.getFieldValue("InceptionDate_Ext"), DATE_FMT)
