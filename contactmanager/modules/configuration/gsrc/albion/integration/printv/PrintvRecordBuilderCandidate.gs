...
/*
 * PrintvRecordBuilderCandidate - Phase 2 CANDIDATE record builder for the PRINTV feed. NOT authoritative anywhere.
 *
 * Same bytes as PrintvRecordBuilder, expressed as a field layout instead of a hand-transcribed
 * StringBuilder sequence. It is reached only through PrintvRecordBuilderShadow, i.e. only when the
 * dormant "shadow.printvrecordbuilder" flag is turned on in a non-production environment; the
 * legacy PrintvRecordBuilder output is what is returned and what production uses.
 *
 * Every documented quirk is preserved on purpose (fixed 250-byte record and its length
 * guard, silent truncation, COBOL trailing-overpunch negatives, PolicyNumber_Ext rendered
 ...
 */
class PrintvRecordBuilderCandidate {
  ...
  public static function buildRecord(src : KeyableBean) : String {
    return new FixedWidthRecordWriter(RECORD_LENGTH, FILLER_CHAR)
        ...
        .number(src.getFieldValue("RiskPostcode_Ext"), 15)
        .date(src.getFieldValue("PolicyNumber_Ext"), DATE_FMT)
