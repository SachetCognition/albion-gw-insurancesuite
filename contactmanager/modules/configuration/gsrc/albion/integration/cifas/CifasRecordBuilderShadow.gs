...
/*
 * CifasRecordBuilderShadow - dormant shadow / cut-over seam for the CIFAS feed.
 *
 ...
 *     pass-through to CifasRecordBuilder.buildRecord, byte-for-byte, no comparison work.
 *
 * Flags: co.feature.shadow.cifasrecordbuilder.enabled and co.feature.cutover.cifasrecordbuilder.enabled
 * (+ optional .brands / .brand.<CODE>.enabled overrides).
 */
class CifasRecordBuilderShadow {

  public static final var CENTRE : String = AlbionFeatureFlags.CENTRE_CONTACTMANAGER
  public static final var FEATURE : String = "shadow.cifasrecordbuilder"
  public static final var CUTOVER_FEATURE : String = "cutover.cifasrecordbuilder"
  public static final var FEED : String = "CIFAS"
  ...
  public static final var BRAND_DRIFT_NOTE : String =
      "brand codes are maintained in CifasRecordBuilder.brandMap() AND integration/polaris/mappings/brand_xref.csv; they have drifted before (AGI-5452 / AGI-30921); convergence is the separate brand stream"
  ...
  public static function buildRecord(src : KeyableBean, recorder : ShadowDiffRecorder) : String {
    ...
    if (AlbionFeatureFlags.isEnabledForBrand(CENTRE, CUTOVER_FEATURE, brandCode)) {
      return CutoverRouter.route(FEED, brandCode, recorder, BRAND_DRIFT_NOTE,
          \ -> CifasRecordBuilder.buildRecord(src),
          \ -> CifasRecordBuilderCandidate.buildRecord(src))
    }
    if (not AlbionFeatureFlags.isEnabledForBrand(CENTRE, FEATURE, brandCode)) {
      return CifasRecordBuilder.buildRecord(src)   // dormant default: legacy only
    }
