...
/*
 * CueRecordBuilderShadow - dormant shadow / cut-over seam for the CUE feed.
 *
 ...
 *     pass-through to CueRecordBuilder.buildRecord, byte-for-byte, no comparison work.
 *
 * Flags: co.feature.shadow.cuerecordbuilder.enabled and co.feature.cutover.cuerecordbuilder.enabled
 * (+ optional .brands / .brand.<CODE>.enabled overrides).
 */
class CueRecordBuilderShadow {

  public static final var CENTRE : String = AlbionFeatureFlags.CENTRE_CONTACTMANAGER
  public static final var FEATURE : String = "shadow.cuerecordbuilder"
  public static final var CUTOVER_FEATURE : String = "cutover.cuerecordbuilder"
  public static final var FEED : String = "CUE"
  ...
  public static final var BRAND_DRIFT_NOTE : String =
      "brand codes are maintained in CueRecordBuilder.brandMap() AND integration/polaris/mappings/brand_xref.csv; they have drifted before (AGI-5452 / AGI-30921); convergence is the separate brand stream"
  ...
  public static function buildRecord(src : KeyableBean, recorder : ShadowDiffRecorder) : String {
    ...
    if (AlbionFeatureFlags.isEnabledForBrand(CENTRE, CUTOVER_FEATURE, brandCode)) {
      return CutoverRouter.route(FEED, brandCode, recorder, BRAND_DRIFT_NOTE,
          \ -> CueRecordBuilder.buildRecord(src),
          \ -> CueRecordBuilderCandidate.buildRecord(src))
    }
    if (not AlbionFeatureFlags.isEnabledForBrand(CENTRE, FEATURE, brandCode)) {
      return CueRecordBuilder.buildRecord(src)   // dormant default: legacy only
    }
