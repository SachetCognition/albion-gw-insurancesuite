...
/*
 * CueRecordBuilderCandidate - Phase 2 CANDIDATE record builder for the CUE feed. NOT authoritative anywhere.
 *
 * Same bytes as CueRecordBuilder, expressed as a field layout instead of a hand-transcribed
 * StringBuilder sequence. It is reached only through CueRecordBuilderShadow, i.e. only when the
 * dormant "shadow.cuerecordbuilder" flag is turned on in a non-production environment; the
 * legacy CueRecordBuilder output is what is returned and what production uses.
 *
 ...
 */
class CueRecordBuilderCandidate {
  ...
  public static function buildRecord(src : KeyableBean) : String {
    return new FixedWidthRecordWriter(RECORD_LENGTH, FILLER_CHAR)
        ...
        .text(src.getFieldValue("NINumber_Ext") as String, 10)
        .number(src.getFieldValue("AnnualPremium_Ext"), 11)
