...
/*
 * PolarisPartyRecordBuilderShadow - dormant shadow / cut-over seam for the POLARIS_MF feed.
 *
 ...
 *     pass-through to PolarisPartyRecordBuilder.buildRecord, byte-for-byte, no comparison work.
 *
 * Flags: co.feature.shadow.polarispartyrecordbuilder.enabled and co.feature.cutover.polarispartyrecordbuilder.enabled
 * (+ optional .brands / .brand.<CODE>.enabled overrides).
 */
class PolarisPartyRecordBuilderShadow {

  public static final var CENTRE : String = AlbionFeatureFlags.CENTRE_CONTACTMANAGER
  public static final var FEATURE : String = "shadow.polarispartyrecordbuilder"
  public static final var CUTOVER_FEATURE : String = "cutover.polarispartyrecordbuilder"
  ...
  public static final var BRAND_DRIFT_NOTE : String =
      "brand codes are maintained in PolarisPartyRecordBuilder.brandMap() AND integration/polaris/mappings/brand_xref.csv; they have drifted before (AGI-5452 / AGI-30921); convergence is the separate brand stream"
  ...
  public static function buildRecord(src : KeyableBean, recorder : ShadowDiffRecorder) : String {
    ...
    if (AlbionFeatureFlags.isEnabledForBrand(CENTRE, CUTOVER_FEATURE, brandCode)) {
      return CutoverRouter.route(FEED, brandCode, recorder, BRAND_DRIFT_NOTE,
          \ -> PolarisPartyRecordBuilder.buildRecord(src),
          \ -> PolarisPartyRecordBuilderCandidate.buildRecord(src))
    }
    if (not AlbionFeatureFlags.isEnabledForBrand(CENTRE, FEATURE, brandCode)) {
      return PolarisPartyRecordBuilder.buildRecord(src)   // dormant default: legacy only
    }
