...
- Guard: `tools/ci/verify_phase1_scaffold.py`, which fails the stage if a listed test class has been deleted or renamed; if a committed `.golden` fixture and the literal asserted in its characterization test have drifted apart, in either the expected record or the inputs; if a fixture stops describing a 512-byte record; if the committed fixture count drops below `MIN_FIXTURES`; if a shadow flag is enabled in any committed environment file; if either Jenkinsfile or the runner script itself re-acquires a failure-swallowing construct (`|| true`, `set +e`, `catchError`, `returnStatus: true`, `ignoreFailure`); or if a pipeline stops invoking `run-golden-master.sh` on an executable (non-comment) line. The guard was mutation-tested: each of those conditions was injected individually and confirmed to exit non-zero.
